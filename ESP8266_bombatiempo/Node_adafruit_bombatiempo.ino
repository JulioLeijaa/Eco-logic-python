...
Adafruit_MQTT_Publish pbombatiempo = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/bombatiempo",MQTT_QOS_1);
...
Adafruit_MQTT_Subscribe sbombatiempo = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/bombatiempo",MQTT_QOS_1);
/*************************** Sketch Code ************************************/
...
void digitalCallback2(char *data, uint16_t len) {
  Serial.print("Bomba value is: ");
  Serial.println(data);
     //String message = String(data);
      int x; 
...
void setup() {
  ...
  sbombatiempo.setCallback(digitalCallback2);
  mqtt.subscribe(&sbombatiempo);
  }
